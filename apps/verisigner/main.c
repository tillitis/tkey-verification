...
int main(void)
{
	...
	for (;;) {
		// blocking; flashing cyan (green+blue) while waiting for cmd
		in = readbyte_ledflash(LED_GREEN | LED_BLUE, 900000);
		*led = led_steady;
		qemu_puts("Read byte: ");
		qemu_puthex(in);
		qemu_lf();
		...
		case APP_CMD_GET_FIRMWARE_HASH:
			qemu_puts("APP_CMD_GET_FIRMWARE_HASH\n");
			if (hdr.len != 32) {
				rsp[0] = STATUS_BAD;
				appreply(hdr, APP_RSP_GET_FIRMWARE_HASH, rsp);
				break;
			}

			uint32_t fw_size = cmd[1] + (cmd[2] << 8) +
					   (cmd[3] << 16) + (cmd[4] << 24);

			if (fw_size == 0 || fw_size > 8192) {
			...
			rsp[0] = STATUS_OK;
			crypto_sha512(&rsp[1], (void *)TK1_ROM_BASE, fw_size);
			appreply(hdr, APP_RSP_GET_FIRMWARE_HASH, rsp);
			break;
