...
func (s Signer) GetFirmwareHash(firmwareSize uint32) ([]byte, error) {
	id := 2
	tx, err := tk1.NewFrameBuf(cmdGetFirmwareHash, id)
	if err != nil {
		return nil, fmt.Errorf("NewFrameBuf: %w", err)
	}

	tx[2] = byte(firmwareSize)
	tx[3] = byte(firmwareSize >> 8)
	tx[4] = byte(firmwareSize >> 16)
	tx[5] = byte(firmwareSize >> 24)
	tk1.Dump("GetFirmwareHash tx", tx)
	if err = s.tk.Write(tx); err != nil {
		return nil, fmt.Errorf("Write: %w", err)
	}
